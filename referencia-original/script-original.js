...
const META_REFERIDOS_CARTON_GRATIS = 5;
...
async function actualizarProgramaReferidos(cedula) {
  ...
  try {
    const { data, error } = await supabase
      .from('inscripciones')
      .select('cedula, referido, estado')
      .eq('referido', cedulaLimpia)
      .eq('estado', 'aprobado');
    ...
    const amigosUnicos = new Set(
      (data || [])
        .map(item => normalizarCedulaReferidos(item.cedula))
        .filter(amigo => amigo && amigo !== cedulaLimpia)
    );

    const totalAprobados = amigosUnicos.size;
